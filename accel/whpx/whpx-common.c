...
static void whpx_do_set_phys_mem(MemoryRegionSection *section, bool add)
{
    whpx_slot *mem;
...
/* TODO: Arm-specific function for this */
static void whpx_process_gic_dist_section(MemoryRegionSection *area, bool add)
{
    HRESULT hr;
    ...
    QLIST_FOREACH(deferred, &whpx->early_mem_regions, list_links, next) {
        whpx_set_phys_mem(&deferred->section, deferred->add);
...
static void whpx_set_phys_mem(MemoryRegionSection *section, bool add)
{
    ...
    if (qatomic_read(whpx->atomic_partition_set_up)) {
         ...
         * has been set up
         */
        /* TODO: Write an arm-specific function for this logic */
